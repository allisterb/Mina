...
open Mina
open Mina.Models
open Mina.NLU.ExpertAI
open Mina.NLU.GoogleHC
...
module Server =        
    ...
    let getUser(user:string) : Async<User option> = 
        pgdb
        |> Sql.query "SELECT * FROM Mina_user WHERE user_name=@u"
    ...
    let addUser (user:string) : Async<Result<unit, string>> =
        pgdb
        |> Sql.query "INSERT INTO public.Mina_user(user_name, last_logged_in) VALUES (@u, @d);"
